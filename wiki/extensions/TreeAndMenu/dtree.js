...
dTree.prototype.toString = function(){
    ...
    if (document.getElementById) {
        ...
        if (this.config.useCookies && this.config.useGetCookies) 
...
dTree.prototype.addNode = function(pNode){
    ...
    for (n; n < this.aNodes.length; n++) {
        ...
        if (this.aNodes[n].pid == pNode.id) {
            ...
            if (cn._hc && !cn._io && this.config.useCookies && this.config.useGetCookies) 
			{
...
dTree.prototype.node = function(node, nodeId){
    ...
    //var str = '<div class="dTreeNode lvl-'+this.aIndent.length+' '+((node._ls)?"last":"")+'">' + this.indent(node, nodeId);
    var str = '<div class="dTreeNode'+ ((node._ls) ? " lastTreeNode" : "") + ((!node._hc) ? " innerNode" : "") + '">' + this.indent(node, nodeId);    
    if (this.config.useIcons) {
    	if(this.config.usealternateicons!=false) //activate this if need to use alternate root icon, bug: 6822,6817
		{this.icon.root = this.config.usealternateicons;}
        if (!node.icon) 
        ...
        str += '<img id="i' + this.obj + nodeId + '" src="' + ((node._io) ? node.iconOpen : node.icon) + '" alt="" '+((this.root.id == node.pid) ? 'class="rootIcon"' : '')+' />';
    ...
    if (node._hc) {
        ...
        str += '<div id="d' + this.obj + nodeId + '" class="clip lvl-' + this.aIndent.length + ((node._ls) ? " lastClip" : "") + ' " style="display:' + ((this.root.id == node.pid || node._io) ? 'block' : 'none') + ';">';
...
dTree.prototype.indent = function(node, nodeId){
    ...
    if (this.root.id != node.pid) {
        ...
        if (node._hc) {
            ...
            str += '<a href="javascript: ' + this.obj + '.o(' + nodeId + ');" class="plusicon"><img id="j' + this.obj + nodeId + '" src="';
